...
#数据集格式为VOC格式，含输入图片和标签
## .jpg图片
## 灰度图自动转RGB
## 大小随意，自动resize
## 标签的像素点值 = 像素点所属种类！！！（background = 0）

# loss损失值
## 用于判断是否收敛，大小在于计算方式（可以改动损失函数，美化结果：如/10000）
## 训练过程中，保存在logs文件夹下的loss_%Y_%m_%d_%H_%M_%S文件夹中

# 权值文件
## 保存在logs文件夹中
## n * Step / 1*Epch        1 * 梯度下降 / 1 * Step
if __name__ == "__main__":

    Cuda = True
    seed            = 11    #固定随机种子，获得相同结果/独立训练
    distributed     = False     #指定是否使用单机多卡分布式运行，终端指令仅支持Ubuntu
    ##  DP模式（Data parallel = 单机多卡） ：False
    ### CUDA_VISIBLE_DEVICES=0,1 python train.py（指定显卡）

    ##  DDP模式（指定显卡。）：True
    ### CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train.py
    sync_bn         = False # batch normalization（同步归一化）：DDP模式多卡可用
    fp16            = False # 是否使用混合精度训练，可减少约一半的显存（AMP/auto mixed precision）（fp/float point16/32）
    num_classes = 7+1
    #   num_classes  自己需要的分类个数+1
    # 背景(background)：1
    # 建筑(building)：2
    # 道路(road)：3
    # 水体(water)：4
    ...
    # 森林(forest)：6
    # 农业用地(agriculture)：7
    # 无数据区域(no - data regions)：0（应被忽略）
    backbone    = "vgg"     #   主干网络选择：vgg 或 resnet50
    pretrained  = False #是否使用主干网络的预训练权重
    model_path  = "model_data/unet_vgg_voc.pth"
    #  预训练权重 对不同数据集是通用的，因为特征是通用的;   99%的情况都必须要用，否则主干部分的权值太过随机，特征提取效果不明显，网络训练的结果不好
    ## 主干特征提取网络的权值部分，用于进行特征提取
    ## 训练自己的数据集时提示维度不匹配正常，预测的东西都不一样了自然维度不匹配

    # 训练中断：将model_path设置成logs文件夹下的权值文件，将已经训练了一部分的权值再次载入 + 修改下方的 冻结阶段 或者 解冻阶段 的参数，来保证模型epoch的连续性

    # 是否从0开始训练
    #   否：主干的预训练权值开始训练(设置model_path = ''，pretrain = True，此时仅加载主干）
    #   是：从0开始训练（设置model_path = ''，pretrain = Fasle，Freeze_Train = Fasle，此时从0开始训练，且没有冻结主干的过程）
    input_shape = [512, 512] #32的倍数，确保不出现小数

    #   训练分为两个阶段：冻结阶段（满足机器性能不足的训练需求。需要的显存较小）和解冻阶段
    ...
    lr_decay_type       = 'cos'     #学习率下降方式，可选的有'step'、'cos'

    save_period         = 5     #多少个epoch保存一次权值
    save_dir            = 'logs'     #权值与日志文件保存的文件夹

    eval_flag           = True      #是否在训练时进行评估，评估对象为验证集
    eval_period         = 5         #代表多少个epoch评估一次，不建议频繁的评估（估需要消耗较多的时间，频繁评估会导致训练非常慢）
    ...
    #------------------------------------------------------------------#
    cls_weights     = np.ones([num_classes], np.float32)          #   是否给不同种类赋予不同的损失权值，默认是平衡的。
